...
func main() {
	...
	if len(args) != 1 {
		fmt.Println("Please enter a USD amount:")
		return
	}
	...
	fmt.Printf("$%.2f USD is equal to EUR %.2f\n", amount, rates[EUR]*amount)
	fmt.Printf("$%.2f USD is equal to GRP %.2f\n", amount, rates[GRP]*amount)
	fmt.Printf("$%.2f USD is equal to JPY %.2f\n", amount, rates[JPY]*amount)
}
